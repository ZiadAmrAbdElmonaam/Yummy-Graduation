...
const kitchenSchema = new mongoose.Schema({
    ...
    kitchenCategeory: { type: String, required: true, default: "all", enum: ["vegetarian", "non-vegetarian", "frozen", "all"] },
    kitchenAddress: { type: kitchenAddressSchema, _id: false },
    kitchenPhone: { type: Number, unique: true, required: true, minlength: 5, maxlength: 11 },
    kitchenEmail: {
        type: String, unique: true,
        match: /^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/
    },
    kitchenPassword: { type: String, required: true, minlength: 8 },
    kitchenStatus: { type: String, required: true, default: "closed", enum: ["closed", "open"] },
    kitchenRating: { type: Number, default: 0, max: 5, min: 0 },
    kitchenImage: { type: String, required: true, default: "https://www.w3schools.com/howto/img_avatar.png" },
