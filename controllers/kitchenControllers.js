        ...
        .then((data) => {
            ...
            for (let key in bodyData) {
                ...
                if (key == "kitchenOrders") {
                    if (!data.kitchenOrders.includes(bodyData.kitchenOrders) && bodyData.kitchenOrders != null && bodyData.kitchenOrders.length == 1) {
...
module.exports.deleteKitchenOrders = (req, res, next) => {
    Kitchen.updateOne({ _id: req.params.id }, { $pull: { kitchenOrders: { $eq: req.body.kitchenOrders } } })
        .then(data => {
            console.log(data);
