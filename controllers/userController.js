...
module.exports.getUserOrders = (req, res, next) => {
    ...
    User.findOne({ _id: req.params.id })
        .populate({
            path: "userOrder"
            , populate: [{
                path: "kitchen", model: "kitchens", select: {
                    _id: 0,
                    kitchenName: 1,
                    kitchenAddress: 1,
                    kitchenPhone: 1,
    ...
    User.findOne({ _id: req.params.id })
        .then((data) => {
            ...
            for (let key in bodyData) {
                if (key == "userOrder") {
                    if (!data.userOrder.includes(bodyData.userOrder) && bodyData.userOrder != null && bodyData.userOrder.length == 1) {
...
module.exports.deleteUserOrder = (req, res, next) => {
    User.updateOne({ _id: req.params.id }, { $pull: { userOrder: { $eq: req.body.userOrder } } })
        .then(data => {
            console.log(data);
